...
class LocalSavingBankUseCase(
    private val repository: TransferRepository
) {
    operator fun invoke(
        bank_code: String, bank_acc_num: String, bank_acc_name: String
        ...
        try {
            ...
            val request = BankRequest(
                uuid = uuid, bank_code = bank_code,
                bank_acc_num = bank_acc_num, bank_acc_name = bank_acc_name
            )
            val response = repository.savingLocalBank(request, accessToken)
