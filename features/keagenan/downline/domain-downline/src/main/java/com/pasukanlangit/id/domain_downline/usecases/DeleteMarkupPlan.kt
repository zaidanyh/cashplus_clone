...
class DeleteMarkupPlan(
    private val repository: DownLineRepository
) {
    operator fun invoke(markupCode: String): Flow<DataState<Boolean>> = flow {
        ...
        try {
            ...
            val request = DetailMarkupPlanRequest(uuid = uuid, codeMarkupPlan = markupCode)
            val response = repository.deleteMarkupPlan(request, accessToken)
