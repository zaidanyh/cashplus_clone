...
class JsonArrayOfStringConverter<E>: TypeAdapterFactory {
    override fun <T : Any?> create(gson: Gson?, type: TypeToken<T>): TypeAdapter<T?>? {
        ...
        return JsonArrayOfStringConverter<E?>(elementTypeAdapter).nullSafe() as TypeAdapter<T?>
    }

    private class JsonArrayOfStringConverter<E>(
        ...
        override fun read(`in`: JsonReader?): List<E>? {
            ...
            when(val token: JsonToken = `in`!!.peek()) {
                ...
                JsonToken.STRING -> elementTypeAdapter.read(`in`)?.let { list.add(it) }
