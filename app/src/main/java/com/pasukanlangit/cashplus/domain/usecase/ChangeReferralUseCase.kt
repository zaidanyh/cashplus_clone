...
import com.pasukanlangit.cashplus.model.response.ReferralChangeRequest
...
class ChangeReferralUseCase(
    private val repository: MainRepository
) {
    operator fun invoke(uuid: String, referral: String, accessToken: String): Flow<DataState<UserModel>> = flow {
        emit(DataState.loading())
        try {
            val request = ReferralChangeRequest(referral = referral, uuid = uuid)
            val response = repository.changeReferral(request, accessToken)
