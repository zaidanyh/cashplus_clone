...
class LogoutUseCase(
    ...
    operator fun invoke(uuid: String, accessToken: String): Flow<DataState<UserModel>> = flow {
        emit(DataState.loading())
        try {
            val request = ProfileRequest(uuid = uuid)
            val response = repository.logout(request = request, token = accessToken)
