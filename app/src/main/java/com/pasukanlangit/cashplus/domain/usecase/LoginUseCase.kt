...
import com.pasukanlangit.cashplus.data.toLoginRequestDto
...
class LoginUseCase(
    ...
    operator fun invoke(
        loginRequest: LoginRequest, signature: String
    ): Flow<DataState<UserModel>> = flow {
        emit(DataState.loading())
        try {
            val response = repository.login(loginRequest.toLoginRequestDto(), signature)
