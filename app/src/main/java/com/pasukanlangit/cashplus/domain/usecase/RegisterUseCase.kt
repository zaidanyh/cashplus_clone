...
import com.pasukanlangit.cashplus.data.toRegisterRequestDto
...
class RegisterUseCase(
    ...
    operator fun invoke(request: RegisterRequest): Flow<DataState<OtpResponse>> = flow {
        emit(DataState.loading())
        try {
            val response = repository.register(request.toRegisterRequestDto())
