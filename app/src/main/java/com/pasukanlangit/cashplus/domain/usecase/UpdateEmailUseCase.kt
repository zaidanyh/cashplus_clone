...
import com.pasukanlangit.cashplus.data.toUpdateEmailRequestDto
import com.pasukanlangit.cashplus.domain.model.UpdateEmailRequest
...
class UpdateEmailUseCase(
    ...
    operator fun invoke(
        uuid: String, email: String, accessToken: String
    ): Flow<DataState<UserModel>> = flow {
        emit(DataState.loading())
        try {
            val updateEmailRequest = UpdateEmailRequest(uuid = uuid, email = email)
            val request = updateEmailRequest.toUpdateEmailRequestDto()
            val response = repository.updateEmail(request, accessToken)
