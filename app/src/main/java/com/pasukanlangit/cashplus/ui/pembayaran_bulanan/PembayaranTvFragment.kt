...
import com.pasukanlangit.cashplus.databinding.FragmentPembayaranTvBinding
...
class PembayaranTvFragment : Fragment() {

    private var _binding: FragmentPembayaranTvBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentPembayaranTvBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if(uuid != null && token != null){
            val productRequest = ProductRequest(
                uuid = uuid,
                category = "#TVLANGGANAN",
                kode_provider = "",
                is_faktur = "",
                is_active = "1"
            )

            viewModel.getProductTvKabel(productRequest, token)
        }

        viewModel.tagihanTvKabel.observe(viewLifecycleOwner) { response ->
            if(response.status == Status.SUCCESS) {
                ...
                binding.operatortvListSpinner.adapter = adapter
            }
        }

        with(binding) {
            btnCheckTv.setUpToProgressButton(viewLifecycleOwner)
            btnCheckTv.setOnClickListener {
                val operator = operatortvListSpinner.selectedItem.toString()
                val itemSelected = operatortvListSpinner.selectedItemPosition
                val dest = edtNomerTvkabel.text.toString().trim()
    ...
    private fun observeTAGTvKabel() {
        viewModel.billTrx.observe(viewLifecycleOwner) {
            it.getContentIfNotHandled()?.let { response ->
                when(response.status) {
                    Status.LOADING -> binding.btnCheckTv.showLoading()
                    Status.SUCCESS -> {
                        binding.btnCheckTv.hideProgress(getString(R.string.check))

                        BillDetailFragment.newInstance(
                            destination, response.data,
                            productSelected?.img_url_2?.ifEmpty { productSelected?.img_url },
                            R.drawable.ic_tab_menu_tv_kabel
