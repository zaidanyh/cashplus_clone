...
import com.pasukanlangit.cashplus.databinding.FragmentPembayaranGaspertaminaBinding
...
class PembayaranGasPertaminaFragment : Fragment() {

    private var _binding: FragmentPembayaranGaspertaminaBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentPembayaranGaspertaminaBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if(uuid != null && token != null){
            val productRequest = ProductRequest(
                uuid = uuid,
                category = "#PEMBAYARAN",
                kode_provider = "PERTAGAS",
                is_faktur = "",
                is_active = "1"
            )

            viewModel.getProductPertagas(productRequest, token)
        }

        viewModel.tagihanPertagas.observe(viewLifecycleOwner) { response ->
            if (response.status == Status.SUCCESS) {
                ...
                binding.gaspertaminaListSpinner.adapter = adapter
            }
        }

        with(binding) {
            btnCheckGaspertamina.setUpToProgressButton(viewLifecycleOwner)
            btnCheckGaspertamina.setOnClickListener {
                val operator = gaspertaminaListSpinner.selectedItem.toString()
                val itemSelected = gaspertaminaListSpinner.selectedItemPosition
                val dest = edtNomerPertamina.text.toString().trim()
        ...
        observeTAGGasPertamina()
    }

    private fun observeTAGGasPertamina() {
        viewModel.billTrx.observe(viewLifecycleOwner) {
            it.getContentIfNotHandled()?.let { response ->
                when(response.status) {
                    Status.LOADING -> binding.btnCheckGaspertamina.showLoading()
                    Status.SUCCESS -> {
                        binding.btnCheckGaspertamina.hideProgress(getString(R.string.check))

                        BillDetailFragment.newInstance(
                            destination, response.data,
                            productSelected?.img_url_2?.ifEmpty { productSelected?.img_url },
                            R.drawable.ic_tab_menu_gas_pertamina
