...
import com.pasukanlangit.cashplus.databinding.FragmentPembayaranGasnegaraBinding
...
class PembayaranGasNegaraFragment : Fragment() {

    private var _binding: FragmentPembayaranGasnegaraBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentPembayaranGasnegaraBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (uuid != null && token != null) {
            val productRequest = ProductRequest(
                uuid = uuid,
                category = "#PEMBAYARAN",
                kode_provider = "PERUSAHAAN_GAS_NEGARA",
                is_faktur = "",
                is_active = "1"
            )

            viewModel.getProductGasNegara(productRequest, token)
        }

        viewModel.tagihanGasNegara.observe(viewLifecycleOwner) { response ->
            if (response.status == Status.SUCCESS) {
                ...
                binding.gasnegaraListSpinner.adapter = adapter
            }
        }

        with(binding) {
            btnCheckGasnegara.setUpToProgressButton(viewLifecycleOwner)
            btnCheckGasnegara.setOnClickListener {
                val operator = gasnegaraListSpinner.selectedItem.toString()
                val itemSelected = gasnegaraListSpinner.selectedItemPosition
                val dest = edtNomerGasnegara.text.toString().trim()
    ...
    private fun observeTAGGasNegara() {
        viewModel.billTrx.observe(viewLifecycleOwner) {
            it.getContentIfNotHandled()?.let { response ->
                when(response.status) {
                    Status.LOADING -> binding.btnCheckGasnegara.showLoading()
                    Status.SUCCESS -> {
                        binding.btnCheckGasnegara.hideProgress(getString(R.string.check))

                        BillDetailFragment.newInstance(
                            destination, response.data,
                            productSelected?.img_url_2?.ifEmpty { productSelected?.img_url },
                            R.drawable.ic_tab_menu_gas_negara
