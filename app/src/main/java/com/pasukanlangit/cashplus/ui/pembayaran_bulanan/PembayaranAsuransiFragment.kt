...
import com.pasukanlangit.cashplus.databinding.FragmentPembayaranAsuransiBinding
...
class PembayaranAsuransiFragment : Fragment() {

    private var _binding: FragmentPembayaranAsuransiBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentPembayaranAsuransiBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (uuid != null && token != null) {
            val productRequest = ProductRequest(
                uuid = uuid,
                category = "#PEMBAYARAN",
                kode_provider = "ASURANSI",
                is_faktur = "",
                is_active = "1"
            )

            viewModel.getProductAsuransi(productRequest, token)
        }

        viewModel.tagihanAsuransi.observe(viewLifecycleOwner) { response ->
            if (response.status == Status.SUCCESS) {
                ...
                binding.asuransiListSpinner.adapter = adapter
            }
        }

        binding.btnCheckAsuransi.setUpToProgressButton(viewLifecycleOwner)
        binding.btnCheckAsuransi.setOnClickListener {
            val operator = binding.asuransiListSpinner.selectedItem.toString()
            val itemSelected = binding.asuransiListSpinner.selectedItemPosition
            val dest = binding.edtNomerAsuransi.text.toString().trim()
    ...
    private fun observeTAGAsuransi() {
        viewModel.billTrx.observe(viewLifecycleOwner) {
            it.getContentIfNotHandled()?.let { response ->
                when(response.status) {
                    Status.LOADING -> binding.btnCheckAsuransi.showLoading()
                    Status.SUCCESS -> {
                        binding.btnCheckAsuransi.hideProgress(getString(R.string.check))

                        BillDetailFragment.newInstance(
                            destination, response.data,
                            productSelected?.img_url_2?.ifEmpty { productSelected?.img_url },
                            R.drawable.ic_tab_menu_asuransi
