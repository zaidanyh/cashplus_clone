...
import com.pasukanlangit.cashplus.databinding.FragmentPembayaranSamsatBinding
...
class PembayaranSamsatFragment : Fragment() {

    private var _binding: FragmentPembayaranSamsatBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentPembayaranSamsatBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if(uuid != null && token != null){
            val productRequest = ProductRequest(
                uuid = uuid,
                category = "#PEMBAYARAN",
                kode_provider = "SAMSAT",
                is_faktur = "",
                is_active = "1"
            )

            viewModel.getProductSamsat(productRequest, token)
        }

        viewModel.tagihanSamsat.observe(viewLifecycleOwner) { response ->
        ...
        with(binding) {
            edtSamsatIdentity.filters = arrayOf(InputFilter.LengthFilter(30), inputCharNumberFilter)
            btnCheckSamsat.setUpToProgressButton(viewLifecycleOwner)
            btnCheckSamsat.setOnClickListener {
                val operator = locationListSpinner.selectedItem.toString()
                val itemSelected = locationListSpinner.selectedItemPosition
                val dest = edtSamsatIdentity.text.toString().trim()
    ...
    private fun observeTAGSamsat() {
        viewModel.billTrx.observe(viewLifecycleOwner) {
            it.getContentIfNotHandled()?.let { response ->
                when(response.status) {
                    Status.LOADING -> binding.btnCheckSamsat.showLoading()
                    Status.SUCCESS -> {
                        binding.btnCheckSamsat.hideProgress(getString(R.string.check))

                        BillDetailFragment.newInstance(
                            destination, response.data,
                            productSelected?.img_url_2?.ifEmpty { productSelected?.img_url },
                            R.drawable.ic_tab_menu_samsat
